...
//
/// /    CareerInfo careerInfo = CareerInfo.builder() /        .mentor(user) // User 객체는 예시로 가정 /
///     .startDate(LocalDate.of(2000, 1, 1)) /        .endDate(LocalDate.of(2023, 12, 31)) /
/// .companyName("디지하나") /        .department("개발") /        .task("Software Development") /
/// .certifiacteUrl("https://example.com/certificate") /        .build();
//
